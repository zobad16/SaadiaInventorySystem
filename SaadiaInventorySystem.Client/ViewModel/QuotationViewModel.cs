    ...
    public class QuotationViewModel : BaseViewModel, IViewModel
    {
        ...
        private void WriteFileExcel(string path)
        {
           ...
           try
           {
                ...
                string reff = !string.IsNullOrEmpty(SelectedQuotation.ReferenceNumber) ? SelectedQuotation.ReferenceNumber : SelectedQuotation.QuotationNumber;
                workSheet.Cells["B4"].Value = $"{reff}";
                ...
                foreach(var items in SelectedQuotation.Order.OrderItems)
                {
                    ...
                    {
                        workSheet.Cells[$"F{i}"].Value = $"{ String.Format("{0:0.00}",items.OfferedPrice)}";
                    }
                    else 
                    {
                        workSheet.Cells[$"F{i}"].Value = $"{String.Format("{0:0.00}", items.Inventory.UnitPrice)}";
                    }
        ...
        private void ExcelFooter(ExcelWorksheet workSheet, int i)
        {
            ...
            workSheet.Cells[$"A{ i}"].Value = SelectedQuotation.Note;
            i++;
            i++;
            i++;
            i++;
